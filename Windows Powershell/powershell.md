...
From the official Microsoft page(opens in new tab): “PowerShell is a cross-platform task automation solution made up of a command-line shell, a scripting language, and a configuration management framework.”
...
1. Click on **Applications**, marked with number 1 in the screenshot below.
2. Select **Internet** (number 2), then **Remmina** (number 3).
...
Select the **SSH** option from the dropdown (number 1 in the screenshot below), then paste the target IP: `MACHINE_IP` in the bar at the top (number 2). Finally, click **Enter**.

In the next window, enter the credentials found in the card below, then click **OK**.

## THM Key Credentials

| Username | Password | IP |
|---------|-----------|----|
| `captain` | `JollyR0ger#` | `MACHINE_IP` |
...
The operator `-eq` is part of a set of comparison operators that are shared with other scripting languages (e.g. Bash, Python). To show the potentiality of PowerShell's filtering, we have selected some of the most useful operators from that list:
...
In case we need specific details about the IP addresses assigned to the network interfaces, `Get-NetIPAddress` will show details for all IP addresses configured on the system, including those that are not currently active.
...
You can also view the Alternate Data Streams (ADS) attached to a file through PowerShell as shown below:
...
- `housinginfo` is the Alternate Data Stream (ADS) added to this file. It appears as `house_log.txt:housinginfo`, which means an extra hidden stream named `housinginfo` is attached to this file.
...
To continue building up your command line skills, proceed to the Linux Command Line room, up next in the module.
